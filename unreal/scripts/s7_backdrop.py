...
def make_mountains():
    ...
    # 거리 1.2~1.7km — 더 멀면 저녁 안개에 통째로 지워짐(07-07 확인). 높이도 키움.
    # 북악 주능선 (북쪽 뒤) — 주봉 하나 크게 + 좌우로 낮아짐
    chain("Bukak", 9, 150000, -110000, 175000, 130000, 180, 320, 500, 900)
    peaks.append(("Backdrop_Bukak_Peak", (160000.0, -5000.0), (800.0, 650.0, 430.0)))
    # 북쪽 앞줄 낮은 능선 (1.2km, 겹 보이는 층)
    chain("BukakFront", 6, 120000, -60000, 128000, 80000, 70, 140, 350, 600)
    # 인왕산 (서쪽 1.6~2km) — 넓은 바위산: 큰 봉 + 부속 봉
    chain("Inwang", 7, -20000, -165000, 100000, -195000, 150, 280, 550, 950)
    peaks.append(("Backdrop_Inwang_Peak", (50000.0, -175000.0), (1000.0, 800.0, 380.0)))
    # 남서 원경 (안산 방향)
    chain("Far_SW", 4, -100000, -160000, -40000, -200000, 120, 200, 500, 800)
    return peaks
